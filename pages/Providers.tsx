...
const Providers: React.FC = () => {
  ...
  const [roleFilter, setRoleFilter] = useState<'all' | 'guide' | 'provider'>('provider');
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setAllUsers(all);
      setProviders(providerUsers); // Default to providers
    });
  }, []);
  ...
  useEffect(() => {
    if (roleFilter === 'all') {
      ...
      setProviders(allUsers.filter(u => u.role === UserRole.Guide));
    } else if (roleFilter === 'provider') {
      setProviders(allUsers.filter(u => u.role === UserRole.Provider));
    }
  }, [roleFilter, allUsers]);
  ...
  return (
    <div className="p-4 pb-24 h-full flex flex-col">
      <div className="flex items-center justify-between mb-6 shrink-0">
        <div>
            <h2 className="text-2xl font-bold dark:text-white">{t('providers')}</h2>
            <p className="text-slate-500 text-sm">{t('resorts_camps')}</p>
        </div>
                ...
                providers.map(provider => (
                    ...
                    >
                        <div className="relative shrink-0">
                            <img 
                                src={provider.profilePic || 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_1280.png'} 
                            ...
                            {provider.status === 'pending' ? (
                                ...
                                <div className="absolute -bottom-2 -right-2 bg-green-100 text-green-600 p-1 rounded-full border-2 border-white dark:border-slate-900">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                            <div>
                                <div className="flex justify-between items-start">
                                    <h3 className="font-bold text-lg dark:text-white truncate pr-2">{provider.name}</h3>
                                    {provider.status === 'pending' && <span className="text-[10px] bg-orange-50 text-orange-600 px-1.5 py-0.5 rounded font-bold uppercase">Pending</span>}
                                    {provider.status !== 'pending' && <span className="bg-green-50 text-green-600 text-[10px] px-2 py-1 rounded-lg font-bold uppercase tracking-wider">{t('provider')}</span>}
                                </div>
                                <p className="text-xs text-slate-500 mt-1 line-clamp-2 leading-relaxed">
                                    {provider.serviceDescription || t('provider_desc')}
                                </p>
                            </div>
                            ...
                            <div className="mt-3 flex items-center justify-between">
                                <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-xs font-bold text-slate-700 dark:text-slate-300">
                                        {provider.averageRating ? provider.averageRating : 'New'}
                                    </span>
                                    {provider.reviews && provider.reviews.length > 0 && (
                                        <span className="text-[10px] text-slate-400">({provider.reviews.length})</span>
                                    )}
                                </div>
                                <div className="text-right">
                                     <span className="block text-sm font-bold text-primary">${provider.pricePerDay || 100}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                  ...
                  <Suspense fallback={
                      <div className="w-full h-full rounded-3xl bg-slate-200 dark:bg-slate-800 flex items-center justify-center">
                          <div className="text-center">
                              <div className="animate-spin rounded-full h-12 w-12 border-4 border-slate-300 border-t-green-500 mx-auto mb-4"></div>
